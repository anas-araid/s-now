<?php
  function getMessages($user, $clause){
    include "php/db_connection.php";
    //user è  $user = getUserData($_SESSION['email'], "php/db_connection.php");
    $msgQuery = "SELECT * FROM t_messaggi WHERE (FK_Mittente = '".$user['ID']."') or (FK_Destinatario ='".$user['ID']."') ORDER BY ID DESC $clause";
    ...
    if ($getMsg != null){
      while ($ris = mysqli_fetch_array($getMsg)){
        ...
        <a href="chat.php?username='.$senderData['Email'].'" style="color:#27ae60!important;font-size:20px!important;">'.$senderData['Nome'].': </a>
        <p>'.$ris['Messaggio'].'</p>
